  ...
  if (db) {
    try {
      ...
      if (userId) {
        ...
        if (userSnap.exists) {
          const txs = (userSnap.data().transactions || []);
          // Match on the first 10 chars of the reference (handles both extRef and payheroRef)
          const shortRef = reference.slice(0, 10);
          const matched  = txs.find(tx =>
            tx.type === 'deposit' &&
            tx.status === 'success' &&
            tx.note && tx.note.includes(shortRef)
          );
          if (matched) {
...
app.post('/api/callback', async (req, res) => {
  ...
  try {
    ...
    // If Render restarted and paymentStore is empty, recover userId from Firestore
    if (!userId && db && userRef) {
      try {
        const pendingDoc = await db.collection('pendingPayments').doc(userRef).get();
        if (pendingDoc.exists) userId = pendingDoc.data().userId;
      } catch(e) { /* ignore */ }
    }
    ...
    if (isSuccess && db && userId && amount > 0) {
      ...
      console.warn('[Callback] SUCCESS but no userId — Firestore NOT updated. extRef:', userRef);
    }
